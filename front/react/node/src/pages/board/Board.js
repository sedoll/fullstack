...
const Board = () => {
  ...
  useEffect(() => {
    const getBoard = async () => {
      console.log('post no : '+seq);
      const {data} = await axios.get(`http://localhost:4000/board/get/${seq}`);
      return data;
    }
      ...
      {isLoaded && (
        <div className="board-wrapper">
          ...
          <div className="board-header">
            <div className="board-header-username">{board.nickname}</div>
            <div className="board-header-date">{moment(board.regdate).add(9, "hour").format('YYYY-MM-DD')}</div>
          </div>
          <hr/>
            ...
            <div className="modal-button">
              <Button
                ...
                onClick={async () => {
                  setShow(false);
                  // 모달의 예 버튼 클릭시 게시물 삭제
                  await api.delete(`http://localhost:4000/board/boardDelete/${seq}`);
                  alert("게시물이 삭제되었습니다😎");
